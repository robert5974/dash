...
Selector::Selector(QList<QString> options, QString current, QFont font,
                   Arbiter &arbiter, QWidget *parent, QString placeholder)
  ...
  this->set_state();
  this->current_idx = std::max<qsizetype>(0, this->options.indexOf(current));
...
QSize Selector::sizeHint() const {
  auto it = std::max_element(
      this->options.begin(), this->options.end(),
      [](const auto &a, const auto &b) { return a.size() < b.size(); });

  int base = 32 * this->arbiter.layout().scale;
  int size = (it != this->options.end()) ? it->size() : 0;
  int width = (base * 2) +
              std::max(this->label->width(),
                       QFontMetrics(this->label->font())
                           .horizontalAdvance(QString(size, 'X'))) +
              (12 * 4);
