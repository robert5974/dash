...
QWidget *BluetoothPlayerTab::track_widget() {
  BluezQt::MediaPlayerPtr media_player =
      this->arbiter.system().bluetooth.get_media_player().second;
  ...
  QLabel *title = new QLabel(
      (media_player != nullptr) ? media_player->track().title() : QString(),
      widget);
  title->setIndent(16);
  ...
  connect(&this->arbiter.system().bluetooth,
          &Bluetooth::media_player_track_changed,
  ...
  connect(aa_handler, &AAHandler::aa_media_metadata_update,
          [artist, album, title,
           albumArt](const aasdk::proto::messages::MediaInfoChannelMetadataData
                         &metadata) {
            title->setText(QString::fromStdString(metadata.track_name()));
            if (metadata.has_artist_name())
              artist->setText(QString::fromStdString(metadata.artist_name()));
            if (metadata.has_album_name())
              album->setText(QString::fromStdString(metadata.album_name()));
...
QWidget *BluetoothPlayerTab::controls_widget() {
  ...
  connect(&this->arbiter.system().bluetooth,
          &Bluetooth::media_player_status_changed,
          [play_button](BluezQt::MediaPlayer::Status status) {
            play_button->setChecked(status ==
                                    BluezQt::MediaPlayer::Status::Playing);
          });
...
RadioPlayerTab::RadioPlayerTab(Arbiter &arbiter, QWidget *parent)
    : QWidget(parent), arbiter(arbiter), config(Config::get_instance()),
      plugins(RadioPlayerTab::get_plugins()), loader(),
      tuner(new Tuner(this->arbiter)),
      plugin_selector(new Selector(
          this->plugins.keys(), this->config->get_radio_plugin(),
          this->arbiter.forge().font(14), this->arbiter, nullptr, "unloader")),
  ...
  connect(this->play_button, &QPushButton::clicked, [this](bool checked) {
    if (RadioPlugin *plugin =
            qobject_cast<RadioPlugin *>(this->loader.instance())) {
      if (checked)
        plugin->play();
      else
        plugin->stop();
...
RadioPlayerTab::~RadioPlayerTab() { this->loader.unload(); }
...
QWidget *RadioPlayerTab::tuner_widget() {
  ...
  auto station =
      new QLabel(QString::number(this->tuner->sliderPosition() / 10.0, 'f', 1));
...
QWidget *LocalPlayerTab::playlist_widget() {
  ...
  this->arbiter.forge().iconize("playlist_add", "playlist_add_check",
                                home_button, 32);
  connect(home_button, &QPushButton::clicked, [this](bool checked = false) {
    this->config->set_media_home(checked ? this->path_label->text()
                                         : QDir().absolutePath());
  });
  ...
  connect(tracks, &QListWidget::itemClicked,
          [this, tracks](QListWidgetItem *item) {
            this->playlist_set_current_index(tracks->row(item));
          });
  connect(this->player, &QMediaPlayer::sourceChanged,
          [this, tracks](const QUrl &media) {
            int idx = this->playlist_current_index();
            if (idx < 0)
              return;
            tracks->setCurrentRow(idx);
          });
  connect(folders, &QListWidget::itemClicked,
          [this, folders, tracks, home_button](QListWidgetItem *item) {
            if (!item->isSelected())
              return;
            ...
            home_button->setChecked(this->config->get_media_home() ==
                                    current_path);
          });
...
QWidget *LocalPlayerTab::seek_widget() {
  ...
  slider->setRange(0, 0);
  QLabel *value =
      new QLabel(LocalPlayerTab::durationFmt(slider->value()), widget);
  connect(slider, &QSlider::sliderReleased, [player = this->player, slider]() {
  ...
  connect(this->player, &QMediaPlayer::durationChanged,
          [slider](qint64 duration) {
            slider->setValue(0);
            slider->setRange(0, duration);
          });
  connect(this->player, &QMediaPlayer::positionChanged,
          [slider, value](qint64 position) {
...
QWidget *LocalPlayerTab::controls_widget() {
  ...
  this->arbiter.forge().iconize("play", "pause", play_button, 56);
  connect(play_button, &QPushButton::clicked,
          [player = this->player, play_button](bool checked = false) {
            play_button->setChecked(!checked);
            if (checked)
              player->play();
            else
...
void LocalPlayerTab::populate_dirs(QString path, QListWidget *dirs_widget) {
  ...
  QFileInfoList dirs =
      current_dir.entryInfoList(QDir::AllDirs | QDir::Readable);
...
void LocalPlayerTab::populate_tracks(QString path, QListWidget *tracks_widget) {
  QStringList tracks = QDir(path).entryList(QStringList() << "*.mp3",
                                            QDir::Files | QDir::Readable);
  for (QString track : tracks) {
    QUrl url = QUrl::fromLocalFile(path + '/' + track);
    this->playlist_add_media(url);

    TagLib::FileRef f(
        std::string(path.toStdString() + "/" + track.toStdString()).c_str());
    if (!f.isNull() && f.tag()) {
...
void LocalPlayerTab::playlist_add_media(const QUrl &url) {
  this->playlist_.append(url);
}

void LocalPlayerTab::playlist_clear() {
  this->playlist_.clear();
  this->current_playlist_index_ = -1;
  this->player->stop();
  this->player->setSource(QUrl());
}

void LocalPlayerTab::playlist_set_current_index(int index) {
  if (index >= 0 && index < this->playlist_.size()) {
    this->current_playlist_index_ = index;
    this->play_current();
  }
}

int LocalPlayerTab::playlist_current_index() const {
  return this->current_playlist_index_;
}

void LocalPlayerTab::playlist_next() {
  if (this->playlist_.isEmpty())
    return;

  if (this->current_playlist_index_ + 1 < this->playlist_.size()) {
    this->current_playlist_index_++;
  } else {
    this->current_playlist_index_ = 0; // Loop
  }
  this->play_current();
}

void LocalPlayerTab::playlist_previous() {
  if (this->playlist_.isEmpty())
    return;

  if (this->current_playlist_index_ > 0) {
    this->current_playlist_index_--;
  } else {
    this->current_playlist_index_ = this->playlist_.size() - 1; // Loop
  }
  this->play_current();
}

void LocalPlayerTab::play_current() {
  if (this->current_playlist_index_ >= 0 &&
      this->current_playlist_index_ < this->playlist_.size()) {
    this->player->setSource(this->playlist_[this->current_playlist_index_]);
    this->player->play();
  }
}
