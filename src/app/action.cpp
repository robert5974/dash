...
const QRegularExpression GPIONotifier::GPIOX_REGEX("^gpio\\d+$");
const QString GPIONotifier::GPIO_DIR("/sys/class/gpio");
const QString GPIONotifier::GPIOX_DIR(GPIONotifier::GPIO_DIR + "/%1");
const QString GPIONotifier::GPIOX_VALUE_PATH(GPIONotifier::GPIOX_DIR +
                                             "/value");
const QString GPIONotifier::GPIOX_ACTIVE_LOW_PATH(GPIONotifier::GPIOX_DIR +
                                                  "/active_low");

GPIONotifier::GPIONotifier() : QObject(), watcher() {
  for (auto gpio :
       QDir(this->GPIO_DIR).entryList(QDir::Dirs | QDir::NoDotAndDotDot)) {
    if (this->GPIOX_REGEX.match(gpio).hasMatch() &&
        QFile(this->GPIOX_ACTIVE_LOW_PATH.arg(gpio)).exists())
...
ActionDialog::ActionDialog(Arbiter &arbiter)
    : Dialog(arbiter, true, arbiter.window()), notifier() {
  ...
  connect(&this->notifier, &GPIONotifier::triggered,
          [this](QString gpio) { this->label->setText(gpio); });
}

void ActionDialog::keyPressEvent(QKeyEvent *event) {
  static const auto mod_keys = {Qt::Key_unknown, Qt::Key_Control, Qt::Key_Shift,
                                Qt::Key_Alt, Qt::Key_Meta};

  const auto key = static_cast<Qt::Key>(event->key());
  if (std::any_of(mod_keys.begin(), mod_keys.end(),
...
Action::GPIO::GPIO() : watcher(), value(), active_low(0xFF) {}
...
Action::Action(QString name, std::function<void(ActionState)> action,
               QWidget *parent)
    : QObject(parent), shortcut(parent), gpio(), name_(name), key_(),
      func_(action) {
  connect(&this->gpio.watcher, &QFileSystemWatcher::fileChanged,
          [this, action](QString) {
            if (this->gpio.value.isOpen()) {
              ...
              if (this->gpio.active_low == this->gpio.value.read(1).at(0)) {
                ...
                QTimer::singleShot(
                    300, [this] { this->gpio.watcher.blockSignals(false); });
              } else {
                QString debugStr;
              ...
              QString debugStr;
              QDebug stream(&debugStr);
              stream << this->key_ << ":" << this->gpio.value.fileName()
                     << "is not open"; // temp
              DASH_LOG(info) << "[Action] " << debugStr.toStdString();
            }
          });
  connect(&this->shortcut, &QShortcut::activated,
          [action] { action(ActionState::Triggered); });
}
...
void Action::set(QString key) {
  ...
  if (this->key_.startsWith("gpio")) {
    ...
    this->gpio.value.setFileName(
        GPIONotifier::GPIOX_VALUE_PATH.arg(this->key_));
    if (this->gpio.value.open(QIODevice::ReadOnly)) {
      QFile active_low(GPIONotifier::GPIOX_ACTIVE_LOW_PATH.arg(this->key_),
                       this);
    ...
    if (!this->key_.contains('+')) {
      QString debugStr;
      QDebug stream(&debugStr);
      stream << "[Action]" << this->key_
             << ": single key sequence, setting via eventFilter";
      DASH_LOG(info) << debugStr.toStdString();
      actionEventFilter->eventFilterMap.insert(
          QKeySequence::fromString(this->key_)[0], this);
    } else {
