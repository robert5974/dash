...
"""
This is the script responsible for changing the volume in response to a turn or press
of the rotary encoder.  Credit to https://github.com/savetheclocktower

It turns infinitely in either direction.
Turning it to the right will increase the volume; turning it to the left will
decrease the volume. The rotary encoder can also be pressed like a button in order to
turn muting on or off.

The rotary encoder uses two GPIO pins and we need some extra logic to decode it. The
button push can be treated like an ordinary button. Rather than polling
...
# The two pins that the rotary encoder uses (BCM numbering).
GPIO_A = 26   
GPIO_B = 19

# The pin that the rotary encoder's button is connected to. NOTE: If you have no button, set
# this to None.
GPIO_BUTTON = 13 
...
# greater than zero because sound can get
# completely inaudible before reaching 0%. If you have a hardware amp or
# serious speakers or something like that, your results will vary.
VOLUME_MIN = 20
VOLUME_MAX = 90

# The amount you want one click of the rotary encoder to increase or decrease the
# volume. Non-integer values probably won't work here, but you're welcome
# to try.
VOLUME_INCREMENT = 1
...
# When the rotary encoder is turned, the callback happens in a separate thread. If
# those turn callbacks fire erratically or out of order, we will get confused
# about which direction the rotary encoder is being turned, so we'll use a queue to
...
# process the queue and reset the event. If the rotary encoder is turned very quickly,
...
class Volume:
  ...
  # click of the rotary encoder in either direction, which is why we're doing simple
...
if __name__ == "__main__":
  ...
  # queueing ensures that we won't miss anything if the rotary encoder is turned
  # extremely quickly.
  def on_turn(delta):
    QUEUE.put(delta)
    EVENT.set()
  ...
  while True:
    ...
    # block quietly while waiting for the event to get flagged. When the rotary encoder
