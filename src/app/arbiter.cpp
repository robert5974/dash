...
Arbiter::Arbiter(MainWindow *window)
    : QObject(), window_(window), session_(*this) {}
...
void Arbiter::toggle_mode() {
  auto mode = (this->theme().mode == Session::Theme::Light)
                  ? Session::Theme::Dark
                  : Session::Theme::Light;
  this->set_mode(mode);
}
...
void Arbiter::set_fullscreen(bool fullscreen) {
  ...
  if (fullscreen) {
    ...
    page->container()->reset(); // reinserts widget into container, removing it
                                // from window stack
  }
...
void Arbiter::set_brightness_plugin(QString plugin) {
  if (!(this->system().brightness.plugins().contains(plugin) ||
        (plugin == Session::System::Brightness::AUTO_PLUGIN)))
    return;
...
void Arbiter::decrease_brightness(uint8_t val) {
  this->set_brightness(
      std::min(std::max(76, this->system().brightness.value - val), 255));
}
...
void Arbiter::set_action(Action *action, QString key) {
  int actionId = this->core().action_id(action);
  if (actionId < 0)
    return;

  auto id = QString::number(actionId);
...
QMainWindow *Arbiter::window() { return this->window_; }
