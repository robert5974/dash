...
StepMeter::StepMeter(Arbiter &arbiter, QWidget *parent) : QFrame(parent) {
...
void StepMeter::paintEvent(QPaintEvent *event) {
  ...
  for (int i = 0; i < this->steps; i++) {
    ...
    QPolygonF bar(QRectF((i * (bar_width + spacing)) + spacing,
                         this->height() - bar_height, bar_width, bar_height));

    QPainterPath path;
    path.addPolygon(bar);
    painter.fillPath(path, (this->bars >= i) ? QBrush(this->bar_color)
                                             : QBrush(this->base_color));
  }
}
