...
FullscreenToggler::FullscreenToggler(Arbiter &arbiter, QString name,
                                     QWidget *widget)
    : arbiter(arbiter), name_(name), widget_(widget) {}

NullFullscreenToggler::NullFullscreenToggler(Arbiter &arbiter)
    : QFrame(arbiter.window()), FullscreenToggler(arbiter, "none", this) {}

BarFullscreenToggler::BarFullscreenToggler(Arbiter &arbiter)
    : QFrame(arbiter.window()), FullscreenToggler(arbiter, "bar", this) {
  auto layout = new QHBoxLayout(this);
  layout->setContentsMargins(0, 0, 0, 0);

  auto bar = new QPushButton();
  connect(bar, &QPushButton::clicked,
          [&arbiter] { arbiter.set_fullscreen(false); });
...
ButtonFullscreenToggler::ButtonFullscreenToggler(Arbiter &arbiter)
    : QDialog(arbiter.window(), Qt::FramelessWindowHint |
      ...
      p(this->arbiter.window()->pos()), last_pos(this->p), touch_start(),
      label(new QLabel(" X ")) {
  ...
  connect(&this->arbiter, &Arbiter::brightness_changed,
...
void ButtonFullscreenToggler::mouseMoveEvent(QMouseEvent *event) {
  this->move(event->globalPosition().x() - this->p.x(),
             event->globalPosition().y() - this->p.y());
}
