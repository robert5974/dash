...
OpenAutoWorker::OpenAutoWorker(std::function<void(bool)> callback,
                               bool night_mode, QWidget *frame,
                               Arbiter &arbiter)
    : QObject(qApp), io_service(), work(io_service),
      configuration(Config::get_instance()->openauto_config), tcp_wrapper(),
      ...
      usb_hub(std::make_shared<aasdk::usb::USBHub>(usb_wrapper, io_service,
                                                   query_chain_factory)),
      connected_accessories_enumerator(
          std::make_shared<aasdk::usb::ConnectedAccessoriesEnumerator>(
              usb_wrapper, io_service, query_chain_factory)),
      app(std::make_shared<openauto::App>(io_service, usb_wrapper, tcp_wrapper,
                                          android_auto_entity_factory, usb_hub,
                                          connected_accessories_enumerator)) {
...
void OpenAutoWorker::create_usb_workers() {
  std::function<void()> worker = [this]() {
    ...
    while (!this->io_service.stopped())
      libusb_handle_events_timeout_completed(this->usb_context, &event_timeout,
                                             nullptr);
  };
...
QLayout *OpenAutoPage::Settings::rhd_row_widget() {
  ...
  toggle->setChecked(
      this->config->openauto_config->getHandednessOfTrafficType() ==
  ...
  connect(toggle, &Switch::stateChanged, [config = this->config](bool state) {
    config->openauto_config->setHandednessOfTrafficType(
        state
            ? openauto::configuration::HandednessOfTrafficType::RIGHT_HAND_DRIVE
            : openauto::configuration::HandednessOfTrafficType::
                  LEFT_HAND_DRIVE);
  });
...
QLayout *OpenAutoPage::Settings::frame_rate_row_widget() {
  ...
  fps_30_button->setChecked(this->config->openauto_config->getVideoFPS() ==
                            aasdk::proto::enums::VideoFPS::_30);
  connect(fps_30_button, &QRadioButton::clicked, [config = this->config]() {
    config->openauto_config->setVideoFPS(aasdk::proto::enums::VideoFPS::_30);
  });
  group_layout->addWidget(fps_30_button);

  QRadioButton *fps_60_button = new QRadioButton("60fps", group);
  fps_60_button->setChecked(this->config->openauto_config->getVideoFPS() ==
                            aasdk::proto::enums::VideoFPS::_60);
  connect(fps_60_button, &QRadioButton::clicked, [config = this->config]() {
    config->openauto_config->setVideoFPS(aasdk::proto::enums::VideoFPS::_60);
  });
...
QLayout *OpenAutoPage::Settings::rt_audio_row_widget() {
  ...
  toggle->scale(this->arbiter.layout().scale);
  toggle->setChecked(
      this->config->openauto_config->getAudioOutputBackendType() ==
      openauto::configuration::AudioOutputBackendType::RTAUDIO);
  connect(toggle, &Switch::stateChanged, [config = this->config](bool state) {
    config->openauto_config->setAudioOutputBackendType(
        state ? openauto::configuration::AudioOutputBackendType::RTAUDIO
...
QLayout *OpenAutoPage::Settings::audio_channels_row_widget() {
  ...
  music_button->setChecked(
      this->config->openauto_config->musicAudioChannelEnabled());
  connect(music_button, &QCheckBox::toggled,
          [config = this->config](bool checked) {
            config->openauto_config->setMusicAudioChannelEnabled(checked);
          });
  group_layout->addWidget(music_button);
  group_layout->addStretch(2);

  QCheckBox *speech_button = new QCheckBox("Speech", group);
  speech_button->setChecked(
      this->config->openauto_config->speechAudioChannelEnabled());
  connect(speech_button, &QCheckBox::toggled,
          [config = this->config](bool checked) {
            config->openauto_config->setSpeechAudioChannelEnabled(checked);
          });
...
QLayout *OpenAutoPage::Settings::bluetooth_row_widget() {
  ...
  connect(toggle, &Switch::stateChanged, [config = this->config](bool state) {
    config->openauto_config->setBluetoothAdapterType(
        state ? openauto::configuration::BluetoothAdapterType::LOCAL
...
QLayout *OpenAutoPage::Settings::touchscreen_row_widget() {
  ...
  connect(toggle, &Switch::stateChanged, [config = this->config](bool state) {
    config->openauto_config->setTouchscreenEnabled(state);
  });
  layout->addWidget(toggle, 1, Qt::AlignHCenter);
...
QCheckBox *OpenAutoPage::Settings::button_checkbox(
    QString name, QString key, aasdk::proto::enums::ButtonCode::Enum code) {
  ...
  connect(checkbox, &QCheckBox::toggled,
          [config = this->config, code](bool checked) {
            if (checked) {
              config->openauto_button_codes.push_back(code);
            } else {
              config->openauto_button_codes.erase(
                  std::remove(config->openauto_button_codes.begin(),
                              config->openauto_button_codes.end(), code),
...
QLayout *OpenAutoPage::Settings::buttons_row_widget() {
  ...
  group_layout->addWidget(this->button_checkbox(
      "Enter", "Enter", aasdk::proto::enums::ButtonCode::ENTER));
  group_layout->addWidget(this->button_checkbox(
      "Left", "Left", aasdk::proto::enums::ButtonCode::LEFT));
  group_layout->addWidget(this->button_checkbox(
      "Right", "Right", aasdk::proto::enums::ButtonCode::RIGHT));
  group_layout->addWidget(
      this->button_checkbox("Up", "Up", aasdk::proto::enums::ButtonCode::UP));
  group_layout->addWidget(this->button_checkbox(
...
void OpenAutoPage::init() {
  ...
  std::function<void(bool)> callback = [frame = this->frame](bool active) {
    frame->toggle(active);
  };
  this->worker = new OpenAutoWorker(
      callback, this->arbiter.theme().mode == Session::Theme::Dark, frame,
      this->arbiter);
  ...
  connect(this->frame, &OpenAutoFrame::toggle, [this](bool enable) {
    ...
    if (Config::get_instance()->get_show_aa_connected()) {
      ...
      if (enable)
        icon.addFile(QString(":/icons/%1.svg").arg(this->connected_icon_name),
                     QSize(), QIcon::Active, QIcon::On);
      else
        icon.addFile(QString(":/icons/%1.svg").arg(this->icon_name()), QSize(),
  ...
  connect(&this->arbiter, &Arbiter::mode_changed,
          [this, aa_handler](Session::Theme::Mode mode) {
...
QWidget *OpenAutoPage::connect_msg() {
  ...
  connect(save_button, &QPushButton::clicked, [this]() {
    this->config->openauto_config->setButtonCodes(
        this->config->openauto_button_codes);
