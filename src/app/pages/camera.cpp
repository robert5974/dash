...
CameraPage::CameraPage(Arbiter &arbiter, QWidget *parent)
    : QWidget(parent), Page(arbiter, "Camera", "camera", true, this) {}
...
void CameraPage::init_gstreamer_pipeline(std::string desc, bool sync) {
  ...
  if (this->config->get_cam_overlay()) {
    ...
    pipeline =
        pipeline +
        " ! videoconvert ! rsvgoverlay location=/tmp/dash_camera_overlay.svg "
        "width-relative=" +
        std::to_string(width) + " height-relative=" + std::to_string(height) +
...
QWidget *CameraPage::connect_widget() {
  ...
  QCheckBox *auto_reconnect_toggle =
      new QCheckBox("Automatically Reconnect", this);
  ...
  connect(this, &CameraPage::autoconnect_disabled,
          [auto_reconnect_toggle, this] {
...
CameraPage::VideoContainer::VideoContainer(QWidget *parent, CameraPage *page)
    : QWidget(parent) {
  this->page = page;
}
...
void CameraPage::VideoContainer::resizeEvent(QResizeEvent *event) {
  ...
  if ((this->page->videoWidget_ != nullptr) &&
      (this->page->videoContainer_ != nullptr))
...
void CameraPage::count_down() {
  ...
  this->status->setText(
      this->reconnect_message.arg(this->reconnect_in_secs) +
      ((this->reconnect_in_secs == 1) ? "second" : "seconds"));
...
void CameraPage::populate_local_cams() {
  ...
  if (default_device.isEmpty() && !QMediaDevices::defaultVideoInput().isNull())
    default_device = QMediaDevices::defaultVideoInput().id();

  int i = 0;
  for (auto const &cam : cameras) {
    QString pretty_name = cam.description() + " at " + cam.id();
    this->local_cams.append(QPair<QString, QString>(pretty_name, cam.id()));
    if (cam.id() == default_device)
      this->local_index = i;
    i++;
  }

  if (this->local_cams.isEmpty())
    this->local_cams.append(QPair<QString, QString>(
        QString("<No local cameras found>"), QString()));
}
...
QWidget *CameraPage::network_cam_selector() {
  ...
  connect(input, &QLineEdit::returnPressed, this,
          &CameraPage::connect_network_stream);
...
void CameraPage::connect_network_stream() {
  ...
  DASH_LOG(info) << "[CameraPage] Creating GStreamer pipeline with "
                 << this->config->get_cam_network_url().toStdString();
  std::string pipeline =
      "rtspsrc location=" + this->config->get_cam_network_url().toStdString() +
      " latency=300" + " ! queue " +
      // decodebin does some absolute magic on selecting proper plugins
      // more importantly, it knows just the right settings to apply to these
      // plugins I can recreate the exact same pipeline as it ends up using, and
      // it will be much slower because of the lack of setting tuning.
      " ! decodebin" + "";
  init_gstreamer_pipeline(pipeline, true);
  // emit the connected signal before we resize anything, so that videoContainer
  // has had time to resize to the proper dimensions
  emit connected_network();
  if (videoContainer_ == nullptr) {
    DASH_LOG(info)
        << "[CameraPage] No video container, setting projection fullscreen";
    ...
    DASH_LOG(info) << "[CameraPage] Size: " << videoContainer_->width() << "x"
  ...
  gst_pad_add_probe(convertPad, GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM,
                    &CameraPage::convertProbe, this, nullptr);
  gst_element_set_state(vidPipeline_, GST_STATE_PLAYING);
}

GstPadProbeReturn CameraPage::convertProbe(GstPad *pad, GstPadProbeInfo *info,
                                           void *) {
...
void CameraPage::disconnect_stream() {
  DASH_LOG(info)
      << "[CameraPage] Disconnecting camera and destroying gstreamer pipeline";
  GstElement *capsFilter =
      gst_bin_get_by_name(GST_BIN(vidPipeline_), "mycapsfilter");
  GstPad *convertPad = gst_element_get_static_pad(capsFilter, "sink");
  gst_pad_add_probe(convertPad, GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM,
  ...
  if (this->config->get_cam_autoconnect()) {
    qDebug() << "Camera disconnected. Auto reconnect in"
             << this->config->get_cam_autoconnect_time_secs() << "seconds";
...
void CameraPage::connect_local_stream() {
  ...
  DASH_LOG(info) << "[CameraPage] Creating GStreamer pipeline with "
                 << this->config->get_cam_local_device().toStdString();
  std::string pipeline =
      "v4l2src device=" + this->config->get_cam_local_device().toStdString() +
      " ! capsfilter caps=\"video/x-raw,width=" + std::to_string(res.width()) +
      ",height=" + std::to_string(res.height()) +
      ";image/jpeg,width=" + std::to_string(res.width()) +
      ",height=" + std::to_string(res.height()) + "\"" + " ! decodebin";
...
gboolean CameraPage::busCallback(GstBus *, GstMessage *message, gpointer *) {
  ...
  case GST_MESSAGE_WARNING:
    ...
    DASH_LOG(info) << "[CameraPage] Warning " << err->message << " | Debug "
                   << debug;
