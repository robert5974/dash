...
Tuner::Tuner(Arbiter &arbiter)
    : QSlider(Qt::Orientation::Horizontal), mouse_pos(),
  ...
  for (int i = 0; i <= (ticks * 2); i += 2) {
    ...
    if (station % 10 == 0) {
      if (station != this->value()) {
        ...
        painter.setPen(pen);
        painter.drawText(QPointF(x - (fontMetrics.horizontalAdvance(num) / 2.0),
                                 this->height()),
                         num);
        alpha_offset = 12;
      }
    ...
    painter.setPen(
        QPen(tick_color, 4 * this->scale, Qt::SolidLine, Qt::RoundCap));
    painter.drawLine(QLineF(x, y, x, bottom));
  }

  painter.setPen(
      QPen(this->accent, 6 * this->scale, Qt::SolidLine, Qt::RoundCap));
