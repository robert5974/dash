...
elm327::elm327(QString canInterface, bool bluetooth) {
  DASH_LOG(info) << "[ElM327] Attempting to connect to elm device: "
                 << canInterface.toStdString();
  if (bluetooth) {
    this->adapterType = BT;
    btSocket = new QBluetoothSocket(QBluetoothServiceInfo::RfcommProtocol);
    QObject::connect(btSocket, &QBluetoothSocket::connected, this,
                     &elm327::btConnected);
    QObject::connect(btSocket, &QBluetoothSocket::stateChanged, this,
                     &elm327::socketChanged);
    btSocket->connectToService(
        QBluetoothAddress(canInterface),
...
int elm327::_write(std::string str) {
  str += '\r';
  int size;
  if ((size = (adapterType == BT)
                  ? (::send(btSocket->socketDescriptor(), str.c_str(),
                            str.length(), 0))
                  : (write(this->fd, str.c_str(), str.length()))) < 0) {
...
void elm327::initialize() {
  ...
  std::vector<std::string> cmds = {"ATD",  "ATZ",   "ATE0",  "ATL0",
                                   "ATS0", "AT H0", "ATSP0", "0100"};
...
bool elm327::is_failed_response(std::string str) {
  std::vector<std::string> failed_msgs = {"UNABLE TO CONNECT",
                                          "BUS INIT...ERROR",
                                          "NO DATA",
                                          "STOPPED",
                                          "ERROR",
...
QCanBusFrame elm327::receive() {
  ...
  if (i != std::string::npos)
    resp_str.erase(i, searching_phrase.length());
  resp_str.erase(
      std::remove_if(resp_str.begin(), resp_str.end(),
...
void elm327::registerFrameHandler(int id,
...
std::string elm327::_read() {
  ...
  while (true) {
    ...
    if (((adapterType == BT)
             ? (::recv(btSocket->socketDescriptor(), (void *)buf, 1, 0))
             : (read(this->fd, (void *)buf, 1))) != 1) {
      if (adapterType != BT) {
        ...
        if (bluetooth_watchdog->remainingTime() == 0) {
          DASH_LOG(error)
              << "[ELM327] Bluetooth watchdog expired, disconnecting";
          this->connected = false;
          return "";
        }
        continue;
